...
class GardenManager:
    ...
    class GardenStats:
        ...
        def plant_types(self) -> tuple:
            reg = sum(1 for p in self.plants if p.__class__.__name__ == 'Plant')
            flo = sum(1 for p in self.plants if p.__class__.__name__ == 'FloweringPlant')
            pri = sum(1 for p in self.plants if p.__class__.__name__ == 'PrizeFlower')
            return reg, flo, pri
